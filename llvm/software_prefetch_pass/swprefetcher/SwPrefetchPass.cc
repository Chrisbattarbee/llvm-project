...
namespace {
    ...
    struct SwPrefetchPass : FunctionPass, InstVisitor<SwPrefetchPass> {
        ...
        bool depthFirstSearch(Instruction *I, LoopInfo &LI, Instruction *&Phi, SmallVector<Instruction *, 8> &Instrs,
                              SmallVector<Instruction *, 4> &Loads, SmallVector<Instruction *, 4> &Phis,
            ...
            for (Use *v = u; v < end; v++) {
                ...
                if (p && L &&
                    (p == getCanonicalishInductionVariable(L) || p == getWeirdCanonicalishInductionVariable(L))) {
            ...
            if (found) for (auto q : roundInsts)
                Instrs.push_back(q);
                ...
                for (auto &z : newInsts) {
                    ...
                    if (z == Phis[x]) {
                        ...
                        if (z == getCanonicalishInductionVariable(L))
                            n = dyn_cast<Instruction>(Builder.CreateAdd(Phis[x], Phis[x]->getType()->isIntegerTy(64)
                                                                                 ? ConstantInt::get(
                                            Type::getInt64Ty(M->getContext()), offset) : ConstantInt::get(
                            ...
                            Builder.Insert(n);
                            n->setOperand(n->getNumOperands() - 1,
                        ...
                        if (weird) {
                            ...
                            Instruction *endcast = dyn_cast<Instruction>(
                                    Builder.CreatePtrToInt(size, Type::getInt64Ty(M->getContext())));


                            Instruction *startcast = dyn_cast<Instruction>(
                                    Builder.CreatePtrToInt(getWeirdCanonicalishInductionVariableFirst(L),
                            ...
                            assert(cast);
                            Builder.Insert(cast);
                            Value *sub = Builder.CreateSub(cast,
