...
struct SwPrefetchPass : FunctionPass, InstVisitor<SwPrefetchPass> {
  ...
  bool makeLoopInvariantSpec(Instruction *I, bool &Changed, Loop *L) const {
    ...
    if (!isSafeToSpeculativelyExecute(I) && !I->mayReadFromMemory())
      return false; // hacky af but it works for now.
    ...
    for (Value *Operand : I->operands())
      if (Instruction *i = dyn_cast<Instruction>(Operand))
        if (!makeLoopInvariantSpec(i, Changed, L)) {
          Changed = false;
          return false;
        }
  ...
  bool depthFirstSearch(Instruction *I, LoopInfo &LI, Instruction *&Phi,
  ...
  bool runOnFunction(Function &F) override {
    ...
    for (uint64_t x = 0; x < Loads.size(); x++) {
      ...
      if (!getCanonicalishSizeVariable(L))
        continue;
      ...
      bool tryToPushDown = (LI.getLoopFor(Loads[x]->getParent()) !=
      ...
      for (auto &z : newInsts) {
        ...
        if (z == Phis[x]) {
          ...
          int offset =
              (C_CONSTANT * MaxOffsets[x]) / (MaxOffsets[x] + Offsets[x]);
          ...
          if (weird) {
            ...
            Instruction *endcast = dyn_cast<Instruction>(Builder.CreatePtrToInt(
                size, Type::getInt64Ty(M->getContext())));

            Instruction *startcast =
                dyn_cast<Instruction>(Builder.CreatePtrToInt(
                    getWeirdCanonicalishInductionVariableFirst(L),
                    Type::getInt64Ty(M->getContext())));

            Instruction *valcast = dyn_cast<Instruction>(
                Builder.CreatePtrToInt(n, Type::getInt64Ty(M->getContext())));

            Instruction *sub1 =
                dyn_cast<Instruction>(Builder.CreateSub(valcast, startcast));
            Instruction *sub2 =
                dyn_cast<Instruction>(Builder.CreateSub(endcast, startcast));

            Value *cmp = Builder.CreateICmp(CmpInst::ICMP_SLT, sub1, sub2);
            Instruction *rem =
                dyn_cast<Instruction>(Builder.CreateSelect(cmp, sub1, sub2));

            Instruction *add =
                dyn_cast<Instruction>(Builder.CreateAdd(rem, startcast));

            mod = dyn_cast<Instruction>(
                Builder.CreateIntToPtr(add, n->getType()));

          } else if (size->getType() != n->getType()) {
            Instruction *cast =
                CastInst::CreateIntegerCast(size, n->getType(), true);
            assert(cast);
            Builder.Insert(cast);
            Value *sub = Builder.CreateSub(
                cast, ConstantInt::get(Type::getInt64Ty(M->getContext()), 1));
            ...
            Value *sub =
                Builder.CreateSub(size, ConstantInt::get(n->getType(), 1));
            Value *cmp = Builder.CreateICmp(CmpInst::ICMP_SLT, sub, n);
            mod = dyn_cast<Instruction>(Builder.CreateSelect(cmp, sub, n));
          }
...
void addSwPrefetchPass(const PassManagerBuilder &Builder,
